...
/* There are 2 phases in an initialization. First, initial values are provided to propetties by the class that introduced it. Second, allows each class to customize the properties further. The first phase ensures that every property has been initialized and the second phase ensures customizability of the properties. There are 4 safety checks:
...
class Surprise: Polygon {           //overriding all the designated inits of the superclass
    ...
    override init(name: String, sides: Int) {
        self.surprise = "Surprise"
